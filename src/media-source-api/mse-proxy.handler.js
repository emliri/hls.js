...
export class MSEProxyHandler extends EventHandler {
  constructor (hls) {
    super(hls,
  ...
  onSBUpdateError (event) {
    ...
    this.hls.trigger(Event.ERROR, { type: ErrorType.MEDIA_ERROR, details: ErrorDetail.BUFFER_APPENDING_ERROR, fatal: false });
    ...
    for (let trackName in tracks) {
      if (!sourceBuffer[trackName]) {
        ...
        try {
          ...
          this.hls.trigger(Event.ERROR, { type: ErrorType.MEDIA_ERROR, details: ErrorDetail.BUFFER_ADD_CODEC_ERROR, fatal: false, err: err, mimeType: mimeType });
        }
      }
    }
    ...
    if (Object.keys(sourceBuffer).length) {
      ...
      if (segments && segments.length) {
        ...
        try {
          ...
          segments.unshift(segment);
          let event = { type: ErrorType.MEDIA_ERROR, parent: segment.parent };
          if (err.code !== 22) {
            if (this.appendError) {
              this.appendError++;
            } else {
            ...
            event.details = ErrorDetail.BUFFER_APPEND_ERROR;
            ...
            */
            ...
            this.segments = [];
            event.details = ErrorDetail.BUFFER_FULL_ERROR;
            event.fatal = false;
            hls.trigger(Event.ERROR, event);
          }
        }
